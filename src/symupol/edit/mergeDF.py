...
class MergeDF:
    ...
    def mergeDF(self,comuteIfExist,removeLines):
        ...
        if self.__run:
            if comuteIfExist:
                ...
                self.__logger.log(cl=self,method=sys._getframe(),message="start read df phem")
                df_phem=pd.read_csv(filepath_or_buffer=self.__inputMod,sep=";")
                self.__logger.log(cl=self,method=sys._getframe(),message="start read df traj")
                df_traj=pd.read_csv(filepath_or_buffer=self.__inputCsv,sep=";") #                 print (df_phem)                print (df_traj)=
    ...
    def __removeNotCompleteLines(self,removeLines):
        if removeLines:
            ...
            self.__logger.log(cl=self,method=sys._getframe(),message="We do not remove incomplete lines. We copy the "+self.__config.pathOutputMergedTmp +" file to "+self.__config.pathOutputMerged)
